...
total_length = length(names)*(length(names)-1)/2;
...
for i = 1:(length(values) - 1)
   for j = (i+1):(length(values))
       t = t + 1;
       str = ['correlation between ',names{i},' and ',names{j}];
       % In order to get a higher accuracy, resample the data to the same time scale
       [ new_data1, new_data2 ] = union_sample({times{i},values{i}}, {times{j},values{j}}, 60);
%        plot_info(new_data1,new_data2);
       % determine if it's correlated
       ifcorr = 'no';
       [XCF,lags,bounds] = crosscorr(new_data1{2},new_data2{2});
